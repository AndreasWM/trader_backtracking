...
where symbol = 'ADI'
  and PRICE_DATE = to_date('2026.06.24','yyyy.mm.dd')
;
...
gesamt_gewinn as (
  select g.*, exp(sum(ln(1 + gewinn_prozent/100)) over (order by datum)) as wert
    from gewinn g
--   where symbol in ('SNDK', 'MU', 'LITE', 'WDC', 'BE', 'STX', 'INTC', 'CIEN', 'NBIS', 'ASX')
),
letztes_datum as (
  select symbol, max(datum) letztes from gesamt_gewinn
    group by symbol
)
select gg.*
  from gesamt_gewinn gg
  join letztes_datum ld on gg.symbol = ld.symbol and gg.datum = ld.letztes
 where gg.symbol in ('TEAM')
  order by gg.symbol
;

select symbol, datum, preis, market_cap, signal
  from vw_signale
-- where datum between to_date('2026.01.01','yyyy.mm.dd') and to_date('2026.08.04','yyyy.mm.dd')
 where symbol in ('TEAM')
  order by symbol, datum
;
